...
lxc launch ubuntu:24.04 control-plane-01 --cloud-init=cloud-init.yaml
lxc config set control-plane-01 limits.cpu 2
lxc config set control-plane-01 limits.memory 8GB
# Set the root disk quota to 20GB (requires LXD 4.0+)
lxc config device set control-plane-01 root size 20GB
# Mount the current project directory to /home/ubuntu/k8s-project inside the container
...
for node in worker-01 worker-02 worker-03; do
  lxc launch ubuntu:24.04 $node --cloud-init=cloud-init.yaml
